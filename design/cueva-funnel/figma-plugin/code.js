...
let DISPLAY = { family: 'Newsreader', style: 'Regular' };
let SANS_R = { family: 'Archivo', style: 'Regular' };
let SANS_M = { family: 'Archivo', style: 'Medium' };
let SANS_S = { family: 'Archivo', style: 'SemiBold' };
async function tryFont(face) {
    try {
...
async function ensureFonts() {
    ...
    if (!okR || !okM || !okS) {
        ...
        await tryFont(SANS_R);
        await tryFont(SANS_M);
        await tryFont(SANS_S);
...
function V(o) { return Object.assign({ k: 'v' }, o); }
function H(o) { return Object.assign({ k: 'h' }, o); }
function T(o) {
    return Object.assign({ k: 'txt', font: 'r' }, o);
}
function R(o) { return Object.assign({ k: 'rect' }, o); }
function S(svg, w, h, name) { return { k: 'svg', svg: svg, w: w, h: h, name: name }; }
// ---------- renderer ----------
function faceFor(f) {
    if (f === 'display')
        return DISPLAY;
...
function render(spec) {
    if (spec.k === 'txt') {
        ...
        if (spec.ls !== undefined)
            t.letterSpacing = { value: spec.ls, unit: 'PERCENT' };
        t.fills = [solid(spec.color)];
        t.textAlignHorizontal = spec.align || 'LEFT';
        t.textAutoResize = 'HEIGHT';
        if (spec.w)
            t.resize(spec.w, t.height);
        if (spec.name)
            t.name = spec.name;
        if (spec.spans) {
            for (const sp of spec.spans) {
                ...
                if (sp.start < end)
                    t.setRangeFills(sp.start, end, [solid(sp.color)]);
            }
        }
        return t;
    }
    ...
    f.paddingBottom = p[2];
    f.paddingLeft = p[3];
    f.primaryAxisAlignItems = (spec.main || 'MIN');
    f.counterAxisAlignItems = (spec.cross || 'MIN');
    ...
    for (const kid of spec.kids) {
        ...
        f.appendChild(node);
        const anyKid = kid;
        if (anyKid.stretch && 'layoutAlign' in node)
            node.layoutAlign = 'STRETCH';
        if (anyKid.grow && 'layoutGrow' in node)
            node.layoutGrow = anyKid.grow;
    }
...
const H1_SPANS = (function () {
    const s = COPY.h1;
    const a = s.indexOf('$1,800/Month Income Stream');
    const b = s.indexOf('8 to 12 Weeks');
    const out = [];
    if (a >= 0)
        out.push({ start: a, end: a + '$1,800/Month Income Stream'.length, color: C.meadow });
    if (b >= 0)
        out.push({ start: b, end: b + '8 to 12 Weeks'.length, color: C.meadow });
    return out;
})();
// ---------- reusable pieces ----------
function eyebrow(s, color, ls, size) {
...
function btn(label, dark, w, fs, padV, padH, center) {
    return H({
...
function slot(w, h, label, note, tone, iconPaths) {
...
function trustItem(label, paths, w) {
    return H({
        ...
        kids: [
            ...
            T({ s: label, font: 'm', size: 15, lh: 21, color: C.ink, grow: 1 }),
        ],
    });
}
function numberedRow(n, body, w, lineColor, fs) {
    return H({
        ...
        kids: [
            ...
            T({ s: body, size: fs, lh: Math.round(fs * 1.65), color: C.body, grow: 1 }),
        ],
    });
}
function tick(body, w) {
    return H({
        name: 'Tick', gap: 14, pad: [18, 0, 18, 0], cross: 'CENTER', w: w,
        kids: [
            S(ico(P_CHECK, C.meadow, 18), 18, 18, 'Check'),
            T({ s: body, size: 16, lh: 24, color: C.bodyDark, grow: 1 }),
        ],
    });
}
function field(label, placeholder, hint, w, isCal) {
    const kids = [
    ...
    if (isCal) {
        ...
        kids.push(H({
            ...
            kids: [
                H({ name: 'Yes', bg: C.white, h: 54, main: 'CENTER', cross: 'CENTER', grow: 1, kids: [T({ s: 'Yes', size: 16, lh: 24, color: C.body })] }),
                H({ name: 'No', bg: C.white, h: 54, main: 'CENTER', cross: 'CENTER', grow: 1, kids: [T({ s: 'No', size: 16, lh: 24, color: C.body })] }),
            ],
        }));
    }
    if (hint)
        kids.push(T({ s: hint, size: 13, lh: 20, color: C.muted, stretch: true, name: 'Hint' }));
    ...
    const booking = V({
        ...
        kids: [
            ...
            H({ name: 'Split', gap: 56, w: INNER, kids: [
                    V({ name: 'Form card', bg: C.paper, w: 636, pad: [46, 46, 46, 46], gap: 24, kids: [].concat(COPY.fields.map((f, i) => field(f[0], f[1], f[2], formW, i === 6)), [
                            btn(COPY.ctaMain, false, formW, 17, 25, 28, true),
                            H({ name: 'Privacy', gap: 11, w: formW, kids: [
                                    S(ico(P_LOCK, C.slate, 16), 16, 16, 'Lock'),
                                    T({ s: COPY.privacy, size: 14, lh: 23, color: C.body, grow: 1 }),
                                ] }),
                        ]) }),
                    V({ name: 'Walkthrough', gap: 28, w: 548, kids: [
                        ...
                        ].concat(COPY.ticks.map((t) => V({ name: 'Tick row', w: 548, kids: [rule(548, C.lineDark), tick(t, 548)] }))).concat([rule(548, C.lineDark)]) }),
                ] }),
        ],
    });
    const footer = V({
...
function mobilePage() {
    ...
    const trust = V({
        ...
        kids: COPY.trust.map((t, i) => V({ name: 'Item', w: INNER, kids: [].concat(i > 0 ? [rule(INNER, C.lineSage)] : [], [V({ pad: [14, 0, 14, 0], w: INNER, kids: [trustItem(t, [P_SHIELD, P_TAG, P_HOME, P_RAIN][i], INNER)] })]) })),
    });
    const problem = V({
    ...
    const shift = V({
        ...
        ].concat(COPY.shift.map((s) => V({ name: 'Statement', w: INNER, kids: [
                rule(INNER, C.lineDark),
    ...
    const booking = V({
        ...
        ].concat(COPY.ticks.map((t) => V({ name: 'Tick row', w: INNER, kids: [rule(INNER, C.lineDark), tick(t, INNER)] }))).concat([
            rule(INNER, C.lineDark),
            V({ name: 'Form card', bg: C.paper, w: INNER, pad: [26, 26, 26, 26], gap: 20, kids: [].concat(COPY.fields.map((f, i) => field(f[0], f[1], f[2], mFormW, i === 6)), [
                    btn(COPY.ctaMain, false, mFormW, 14, 22, 18, true),
                    H({ name: 'Privacy', gap: 10, w: mFormW, kids: [
                            S(ico(P_LOCK, C.slate, 15), 15, 15, 'Lock'),
                            T({ s: COPY.privacy, size: 13, lh: 21, color: C.body, grow: 1 }),
                        ] }),
                ]) }),
        ]),
    });
